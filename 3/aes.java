...
public class aes {
    ...
    private static String stateToHex(byte[][] s) {
        StringBuilder sb = new StringBuilder();
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) sb.append(String.format("%02X", s[r][c] & 0xFF));
            if (r < 3) sb.append(' ');
        }
        return sb.toString();
    }
    ...
    private static int mul(int a, int b) {
        int res = 0; int aa = a & 0xFF; int bb = b & 0xFF;
        for (int i = 0; i < 8; i++) {
            if ((bb & 1) != 0) res ^= aa;
    ...
    private static void mixColumns(byte[][] s) {
        for (int c=0;c<4;c++){
            ...
            s[0][c]=(byte)r0; s[1][c]=(byte)r1; s[2][c]=(byte)r2; s[3][c]=(byte)r3;
        }
    }
    ...
    private static void addRoundKey(byte[][] s, byte[] rk) {
        for (int c=0;c<4;c++) for (int r=0;r<4;r++) s[r][c]=(byte)((s[r][c]&0xFF)^(rk[c*4+r]&0xFF));
    }
    ...
    private static byte[] decryptBlock(byte[] in, byte[] expandedKey, int blockIndex) {
        ...
        addRoundKey(state, getRoundKey(expandedKey, Nr)); System.out.println("After AddRoundKey (Round " + Nr + "): " + stateToHex(state));
    ...
    private static byte[] parseKeyInput(String keyInput) {
        ...
        if (kb.length < 16) {
            ...
            System.arraycopy(kb,0,padded,0,kb.length);
            return padded;
        } else {
    ...
    public static void main(String[] args) {
        ...
        String cipherStr = encryptToString(plaintext, key);
        System.out.println("\nCiphertext:");
        System.out.println(cipherStr);

        System.out.println("\ndecrypting the string");
        String recovered = decryptFromString(cipherStr, key);
